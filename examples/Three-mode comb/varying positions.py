...
taper_positions=np.arange(0,50,5)
MI_tresholds=np.zeros(len(taper_positions))
Kerr_tresholds=np.zeros(len(taper_positions))
for jj,Z_taper in enumerate(taper_positions):
    params['Z_taper']=Z_taper
    SNAP = SNAP_MI_threshold.SNAP_nonlinear_system(params)
    SNAP.calculate_modes()
    min_threshold,kerr_treshold = SNAP.find_min_positive_threshold()
    MI_tresholds[jj]=min_threshold
    Kerr_tresholds[jj]=kerr_treshold
    
plt.figure()
plt.plot(taper_positions,Kerr_tresholds)
plt.plot(taper_positions,MI_tresholds)